...
> **本目录是按 [04-算法与架构决策备忘录](../../02-技术方案.md) 冻结的项目框架**：目录结构与真实 fork 内布局一致（对齐 [03-代码对接计划书](../../03-开发计划.md) §1.1），当前每个模块以 `.md` 规格书形式存在，供后续编码任务（T 系列）直接对接。**本阶段不写实现代码**——每个 .md 含：模块职责、对外接口（函数签名级）、数据契约、伪代码、验收锚点。
>
