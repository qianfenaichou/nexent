...
def test_chinese_policy_only_describes_store_memory():
    ...
    assert "用户提问" in policy
    assert "工具或代码执行结果" in policy
    assert "模型最终回答" in policy
...
def test_english_policy_excludes_search_and_requires_deduplication():
    ...
    assert "Before every final answer" in policy
    assert "must call `store_memory`" in policy
