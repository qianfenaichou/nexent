...
| **评测报告图（消融柱状图/曲线）** | 维度3 | ★★★ | `deliverables/t23-eval-report.png`（2026-09-21 11:54；由 `e2-ablation-report.json` 真实数字绘制，matplotlib；insufficient_data 格明确标注）。⚠️ **2026-09-23 审图判定必须重绘**：副标题 E8 `Δ=null` 与权威 `e2-ablation-paired2.json` 的 `Δ=0.0` 冲突、缺 A1 聚合 acc=0.7241/pass2=0.70、`n=` 语义待注明；E2 全量回填后一并重绘 | 🟡 重绘欠账 |
