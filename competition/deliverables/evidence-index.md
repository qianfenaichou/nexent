...
| **评测报告图（消融柱状图/曲线）** | 维度3 | ★★★ | `deliverables/t23-eval-report.png`（由 `e2-ablation-report.json` 真实数字绘制，matplotlib；insufficient_data 格明确标注） | 🟡 生成中（r19 截图 agent） |
| E1 纯 RAG 基线（acc=0.6667 / pass2=0.65） | 维度3（对照） | ★★★ | `deliverables/e1-baseline-report.json` | ✅ 已有 |
| E2 消融报告（A1/A2/A3/A4 × E8） | 维度3 | ★★★ | `deliverables/e2-ablation-report.json`（**partial:true，4/16 格有数**：A1 F .7857(14) / M .6667(15)；A2 F .75(12)；A4 F .5(6)；其余 insufficient_data，e8=null） | 🟡 待续跑 |
...
| 构建租户图谱（实体/关系/证据链） | 维度2/3 | ★★★ | `kg_graph` + `kg_evidence_t`（doc_id 链）；2026-09-21 快照：10/120 段、40 实体、27 关系（全部 valid_at=2021-04，2020 版指南时代，2024 版事实 0 条）、19 证据行 | 🟡 摄取中（120 段） |
| 踩坑台账 54 条（调试迭代经验素材，2026-09-21 实数） | 维度4 | ★★★ | `docs/pitfalls.md` | ✅ 已有 |
...
| 缺口 | 阻塞源 | 解锁动作 | 状态 |
|---|---|---|---|
| 六类官方截图（本体工作台/决策卡/示例问答≥3/版本对比/Skill 库/评测图） | 前端栈（宿主机 next dev 3000 + uvicorn 5010；r17 实证 stock 镜像无 knowevo 路由） | 已拍 3 张（`t23-shot-ontology-workbench.png` / `t23-shot-ontology-queue.png` / `t23-shot-skill-template.png`，r17）；余 4 类由 r19 截图 agent 用 DB 真实数据（decision_card_t 161 行 / doc_version_diff_t 4 行 / e2 报告）补齐 | 🟡 进行中 |
| 评测报告图 | E2 partial（4/16 格）；e8=null | 摄取 120/120 → `t22-resume.status.json` resume 四段命令 → 回填完整矩阵；r19 已先按现有真实数字出图（`t23-eval-report.png`），缺口格标注 insufficient_data | 🟡 |
