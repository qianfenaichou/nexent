...
| **评测报告图（消融柱状图/曲线）** | 维度3 | ★★★ | `deliverables/t23-eval-report.png`（**v2 重绘 2026-09-23 13:40**，零 LLM；生成器仓外 `~/kw-plot/plot_e2_report.py`）。审图 §13 三处修正已落：E8 标注改用 paired2 终版 **Δ=0.0**（含诚实限制句 + 旧 Δ=+0.3334 不作数）、补 A1 聚合 acc=0.7241/pass2=0.70、`n=` 注明为判定 run 数；数据源双 JSON、12 格 N/A 不画假 0。E2 全量回填后出终版 | ✅ 口径修正版（终版待 E2 全量） |
