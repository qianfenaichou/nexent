...
- 2026-09-23 23:52（**会话 A · 效率实验 τ_a 重定义**）<!-- session:A τa-efficiency -->：新增探针 `deliverables/algorithm-probes/probe_p7_efficiency_minutes.json`（脚本 `competition/experiments/probe_p7_efficiency_minutes.py`，零 LLM / 零 DB / 零网络 / seed 固定 / 复跑逐字段 IDENTICAL）。**口径重定义**：把「半自动化本体构建效率」从 **auto-accept 比例** 改为 **人工分钟数**。**主结论（A × L）**：证据门 `ev_rich` 是主导约束 —— 门因子主效应 **2.31×**（门开 14.55 min / 门关 6.30 min），**排序因子效应 = 0.00 min（结构零）**。**`insufficient_data`**：达到 `Q*` 的累计分钟数不可测（`Quality(·)` 需 `parent`/`also_parent`/`anchor`/`seed_terms`，冻结池全缺，未用 0 占位）。配套：预登记 `.workbuddy-tmp/pre-registration.md`（冻结 `sha256=e559f5a9…`）、S1 前置条件报告 `s1-baseline-readout.md`、严格版设计 `strict-design.md`、替换文本 `replacement-text-s31.md`。**待同步**：`05-计划书 §3.1` 的 τ_a 叙述替换（见 `replacement-text-s31.md`，含 `(1−1/e)` 必改项）
