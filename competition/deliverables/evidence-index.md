...
| **评测报告图（消融柱状图/曲线）** | 维度3 | ★★★ | `deliverables/t23-eval-report.png`（**v2 重绘 2026-09-23 13:40**，零 LLM；生成器仓外 `~/kw-plot/plot_e2_report.py`）。审图 §13 三处修正已落：E8 标注改用 paired2 终版 **Δ=0.0**（含诚实限制句 + 旧 Δ=+0.3334 不作数）、补 A1 聚合 acc=0.7241/pass2=0.70、`n=` 注明为判定 run 数；数据源双 JSON、12 格 N/A 不画假 0。E2 全量回填后出终版 | ✅ 口径修正版 v3（visual-judge **pass**；终版待 E2 全量）
...
| vision agent 逐张审图 | 截图就位后 | ✅ **已完成（2026-09-23，16/16 张审毕）**：结论见 `screenshot-review.md` §汇总。**必须重拍/重绘 5 张**（T-01-agent-published 空态、T23-home-loggedin 主区空白、t23-shot-ontology-workbench 树区全空、t23-shot-ontology-queue 队列空态、t23-eval-report 需重绘——E8 Δ=null 与 paired2 Δ=0.0 冲突）；带限定 7 张（`None · None`/右缘裁断/dev 徽标等）；其余可用。重拍前置：G6 树渲染修复 + `build_ontology_round` 真跑 ≥30 提案（见 t22/本体诊断线） | ✅ 审毕，重拍欠账 5 张 |
...
- 2026-09-23 23:1x（**会话 B · 算法优化五核前三项**）<!-- session:B algo-opt -->：新增算法层证据 `deliverables/algorithm-probes/probe_p8_alignment_semantic.json` —— **3.2 对齐器换语义聚类**（`backend/services/knowevo/alignment_semantic.py`，设计文档 §5.3 C-1~C-4）。**新数字**：语义聚类 recall **9/9**（保持），阈值敏感性**消除**（legacy 网格 recall 跨度 2/9–9/9 → 新方法 9/9–9/9），声明对齐组 26/517。**口径纪律**：产出量更名为 `alignment_precision_pooled`，并标 **`precision_identifiable=false`（precision 轴 insufficient_data）**——金标非穷尽 ⇒ 无可构造域内阴性集；与 T-21 的 `64/517`（相关率，无假阳计数）**不可直接比较**。阴性对照两臂已区分「无检验力（域外 14 题 pairs_tested=0 退化）」与「有检验力（域内 12 题 slot-fill 0.694）」。同批修正：`06-论文与开源武器库-v2.md` §2/§3.1/§3.2/§4.3/§6.1/§6.3 六处（补录 EvoReasoner 2509.15464 为头号对标、三处引用纠错、许可证红线声明）；代码引用纠错 `pipeline/eval_v1.py:22`、`pipeline/eval_e1.py:16`（误标 `arXiv:2505.23319` → 实为 `2406.12045` τ-bench / `2506.07982` τ²-bench）。**未执行项**：3.1 真实写入秒数、3.3 `EXPLAIN` 与多跳 p95 —— **PG/ES/docker 全不可用**，按工单 §3.5 如实标注并给复现命令，不以 mock 冒充
- 2026-09-24 01:30–02:00（**会话 B · 闭环实测轮**）<!-- session:B algo-opt closure -->：§5.4 与 §6.6 的未实测项**全部闭环**（本机首次具备真实 PG：conda-forge PostgreSQL 16.15，`127.0.0.1:5434`，因沙箱拦截 `setuid/setgid` 而用 user namespace 启动）。**产出**：`docs/verification-reports/sessionB-closure-pg-bench-2026-09-24.md`（证据包）+ `/home/qianqian/bench-runs/`（逐次耗时 JSON、EXPLAIN 原文、PG 语句日志）。**核心数字**：写路径改前 **124.060s** → 改后 **2.291s**（**54.15×**，≤10s 达标）；含 kw_011 四索引 2.255s（附加成本不可测）；多跳 p95 关 57.08ms / 开 70.13ms（**恶化 23%** → 不翻开关）；谓词等价性对称差 **0**；`ix_kr_valid_range` / `ix_ke_valid_range` **被规划器真实选中**。**🔴 同时暴露上一轮交付代码的致命缺陷**：批量写入的存在性查找未分块，3 万边时 PG `StatementTooComplex: stack depth limit exceeded`、**写入完全失败**（已修）；`--batch-size` 亦是空操作（已修）。**口径更正 10 项**已回填 receipt §11（三个不存在的开关、§5.2 复杂度、§6.2 过强论证、§6.4 安全前提的实证等）。回归：pytest **721 passed/30 skipped** 逐位一致 + ruff **All checks passed!**。**诚实缺口**：`repeats=3` 第 2 次 12.2s 根因未定位；≥100 万行规模未测；生产真实库的退化行无法验证（本次预检为 0 仅对合成数据集成立）；多跳 p95 绝对值跨机器不可比。
- 更新纪律：每新增/重拍一张截图、每落定一个评测数字，同步更新本表状态列。
- 2026-09-23 23:52（**会话 A · 效率实验 τ_a 重定义**）<!-- session:A τa-efficiency -->：新增探针 `deliverables/algorithm-probes/probe_p7_efficiency_minutes.json`（脚本 `competition/experiments/probe_p7_efficiency_minutes.py`，零 LLM / 零 DB / 零网络 / seed 固定 / 复跑逐字段 IDENTICAL）。**口径重定义**：把「半自动化本体构建效率」从 **auto-accept 比例** 改为 **人工分钟数**。**主结论（A × L）**：证据门 `ev_rich` 是主导约束 —— 门因子主效应 **2.31×**（门开 14.55 min / 门关 6.30 min），**排序因子效应 = 0.00 min（结构零）**。**`insufficient_data`**：达到 `Q*` 的累计分钟数不可测（`Quality(·)` 需 `parent`/`also_parent`/`anchor`/`seed_terms`，冻结池全缺，未用 0 占位）。配套（2026-09-24 晨间从临时区迁入 git 跟踪树，冻结哈希复验一致）：预登记 `deliverables/algorithm-probes/pre-registration.md`（冻结 `sha256=e559f5a9…`）、S1 前置条件报告 `deliverables/algorithm-probes/s1-baseline-readout.md`、严格版设计 `deliverables/algorithm-probes/strict-design.md`、替换文本 `deliverables/algorithm-probes/replacement-text-s31.md`。✅ **已套入（2026-09-24 上午，用户拍板 r1）**：`05-计划书 §3.1` 已替换为 r1 正式版（写入路径 54.15×、门因子 2.31×、排序结构零、质量轴 `insufficient_data` 与解锁路径）；连带同步 §4.3（移除「与 §3.1 共用 VOI 框架」声称）、§8 低资源构建层行、§9 探针清单（补 p6/p7）。备份 `05-计划书-初赛提交版.md.bak-2026-09-24-prer1`；套入脚本 `archive/overnight-2026-09-24/receipts/apply_r1.py`
- 2026-09-24 凌晨–晨间（**夜间永续任务池收口**）<!-- overnight-2026-09-24 -->：①**T1 验证复核**：pytest **721 passed/30 skipped** 逐位一致（= AGENTS.md 基线）+ ruff `All checks passed!` + probe_p7/p8 复跑一致（p8 逐字节同；p7 仅时间戳+CLI 元数据字段异，已查源码 `probe_p7_efficiency_minutes.py:409`）——证据 `archive/overnight-2026-09-24/receipts/T1-{verification,probe-rerun}.md` + `probe-rerun/{p7,p8}_rerun.json`；②**T4 路径有效性**：`deliverables/algorithm-probes/probe_p9_path_validity.json` —— 40 图×200 case 剪枝完备性 **200/200 R==P PASS**、谓词等价 4935 行一致 0 分歧（265 退化行区间形态报错坐实 caveat）；**证明草稿 DRAFT 待人工复核**（`receipts/T4-path-validity-proof-DRAFT.md`，未经人审不得对外引用）；GiST 索引层运行时 EXPLAIN 等价未跑（`insufficient_data`）、「返回剔除归因」API 未实现；③**T5 金标扩容 batch2**：`competition/corpus/guideline_diff_seed_batch2.md` —— 总话题 **45/50**（honest partial），verified/unverified/UNC = 37/8/12，**可评估话题（recall 分母）9→37**、阴性集 **0→12**（precision 轴从 insufficient_data 具备最小构造条件），种子 16 行未动；④**晨间处理**：夜间机制终止并归档（`archive/overnight-2026-09-24/INDEX.md`），冻结证据迁入跟踪树，T4/T5 补登本表与 `cost-ledger.md`，`05:199` AUROC stale 标注改「已核实」。
