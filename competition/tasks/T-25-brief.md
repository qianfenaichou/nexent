...
## 落地后实测（r22 尾段，t23-capture2 复核）
- 运行中前端（:3000）**热加载**了本 UI：面板实测出现「业务时间（可选，如 2020-01-01T00:00）」+「知识版本（可选，如 v1.0.0）」→ 版本对比路径 UI 可达，**无需 rebuild**。
- 截图时的 as_of 建议取图上真实事实时间桶：`as_of=2024-01-01`（排除 t_v_2024-06-01 批次）vs `as_of=2026-01-01`（全含）。
