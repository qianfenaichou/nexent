...
> **2026-09-19 07:0x 逐题补充（主智能体实测 testset-v1-seed.json）**：5 道 V 题中 **仅 V-004** 的 `answer_old != answer_new`（宽松→个体化，对应 2020/2024 两版指南血糖控制目标变更），其余 4 题（V-001 7.0%、V-002 130/80、V-003 胰岛素/短期强化、V-005 5.3）双金标**相同**；且全部 V 题 `key_facts` 为单时点（1-2 条，无 old/new 拆分；V-004 的两条 ['宽松','个体化'] 恰好各对应一个时点）。**对 E8 的含义**：若只靠现有 5 道 V 题，双金标判分可产生 Δ 的样本 n=1，统计上不可用。T-22 执行器扩展（按臂选金标）之外需要二选一并如实登记：①E8 的 Δ 改在 **F 题**（事实题）上测量——版本钉住裁掉的旧事实对 F 题同样生效，样本 5 道；②扩充种子集补 old/new 不同的 V 题（新增题必须可溯两版指南原文并登记来源，反幻觉条款）。建议 ①，成本零、不碰数据。
