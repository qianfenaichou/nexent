...
> **文档状态（2026-09-23 校准）**：官方模板1 要求的六节 + 智能体设计说明（Agent 配置 / 模型 / 工具 / 知识库 / Skill / 调用关系图 / 调试经验 / 附件说明）**已全部成文，无占位符**（仅 Word 封面「学校/队名/队长/手机号」四项个人信息留【】占位待人工填）。每个数字标注来源等级：**A=实测**（有产物可回溯）/ **E=估算** / **L=文献** / **P=计划**。
...
本项目采用**人机协作分层开发**：人类队员负责赛题对齐、算法设计决策、数据与评估金标、最终评审；AI 编码智能体在**接口契约与验收命令约束下**实现模块（任务包-验收-证据三件套管理，规范见 `07-AI代码编写规范与并行开发手册.md`）。全部代码可开源复现，开发过程留有完整台账（**踩坑台账 67 条**、token 成本台账、知识版本演进记录）——这本身即是官方要求项「调试迭代经验」的实据。
...
| `docs/pitfalls.md` | 调试迭代经验台账（**67 条**） |
| `deliverables/*.png` | 界面与示例问答截图 16 张（含**示例问答 ≥3**：`T-27-chat-qa-{1,2,3}.png`，2026-09-23 闭环，见 §6.3） |
...
**已闭环项（如实记录）**：「示例问答截图 ≥3」于 2026-09-23 由 T-27 闭环——后端以 `NEXENT_SANDBOX_DEFAULT_LEVEL=local` + `MINIO_ENDPOINT=http://127.0.0.1:9010` 重启后，3 轮问答走完整 `/api/agent/run` 全部完成（`chat-qa-summary.json` `errors: []`），智能体真实调用自研 KG 工具（MCP 侧 71×`CallToolRequest`），产出 `deliverables/T-27-chat-qa-{1,2,3}.png`；真库 `decision_card_t` 三行逐题可回溯（截图 2 的 UUID 后缀与库行 `348c8a4d-…-d837546b9ec0` 逐字符吻合）。

**当前已知缺口（如实声明，不掩盖）**：
1. E2 四级消融的 **A3（multihop）尚未跑**；E8 配对终版为 Δ=0.0 的 honest null（n=6 小样本，不可反推机制无效）。
2. 120 题全量测试集未建成，当前为 20 题 seed + 8 题判别集。

以上两项均为**已知且已登记**的缺口，交付时以实际完成状态如实呈现。
